...
public class FeedbackService implements BaseService<FeedbackResource> {
    ...
    public FeedbackResource save(FeedbackResource entity) throws GenericException {
        ...
        return Optional.of(repository.save(feedback)).map(feedbackConverter::convertToEntityAttribute).orElseThrow(() -> new RuntimeException("Could not create the feedback."));
    }

    @Override
    public void delete(Long id) {
        Feedback existingFeedback = repository.findById(id)
                .orElseThrow(() -> new RuntimeException("Could not find the feedback."));
